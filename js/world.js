...
export default class World {
    constructor(scene, seed, directionalLight, manager) {
        this.scene = scene;
        this.seed = seed;
        this.directionalLight = directionalLight;
        this.manager = manager; // Armazena o gerenciador
        ...
        this.animalLoaders = {
            mtl: new MTLLoader(this.manager),
    ...
    loadAnimalModel(callback) {
        ...
        this.animalLoaders.mtl.load(mtlFileName, (materials) => {
            ...
            this.animalLoaders.obj.load(objFileName, (object) => {
                ...
                callback();
            }, undefined, (error) => console.error("Falha ao carregar OBJ do animal.", error));
        }, undefined, (error) => console.error("Falha ao carregar MTL do animal.", error));
    }
    ...
    generate() {
        ...
        const terrainMaterial = createTerrainMaterial(this.manager);
        ...
        this.waterMesh = new Water(
            waterGeometry,
            {
                textureWidth: 512,
                textureHeight: 512,
                waterNormals: new THREE.TextureLoader(this.manager).load( 'textures/waternormals.jpg', function ( texture ) {
    ...
    placeInitialTrees() {
        for (let i = 0; i < this.initialTreeCount; i++) this.createTreeAtRandomLocation();
    }

    placeInitialStones() {
        for (let i = 0; i < this.initialStoneCount; i++) this.createStoneAtRandomLocation();
    }
    ...
    createTree(x, y, z) {
        ...
        mtlLoader.load('Lowpoly_tree_sample.mtl', (materials) => {
            ...
            objLoader.load('Lowpoly_tree_sample.obj', (object) => {
                ...
                const collider = new THREE.Mesh( new THREE.BoxGeometry(1.5, 4, 1.5), new THREE.MeshBasicMaterial({ visible: false }) );
                collider.position.copy(treeGroup.position);
                collider.position.y += 2;
                collider.userData.parentObject = treeGroup;
                treeGroup.userData.collider = collider;
    ...
    createStone(x, y, z) {
        const rockObject = createRock(this.scene, new THREE.Vector3(x, y, z), this.manager);
        this.stones.add(rockObject);
        const collider = new THREE.Mesh( new THREE.BoxGeometry(1.5, 1.5, 1.5), new THREE.MeshBasicMaterial({ visible: false }) );
    ...
    createAnimal(position) {
        ...
        animal.mesh.userData.collider = collider;
        this.interactionColliders.add(collider);
        return animal;
    }
    ...
    removeTree(treeObject) {
        if (treeObject.userData.collider) this.interactionColliders.remove(treeObject.userData.collider);
        treeObject.traverse(disposeObject);
        this.trees.remove(treeObject);
    }
    
    removeStone(stoneObject) {
        if (stoneObject.userData.collider) this.interactionColliders.remove(stoneObject.userData.collider);
        stoneObject.traverse(disposeObject);
        this.stones.remove(stoneObject);
    }

    removeAnimal(animalMesh) {
        if (animalMesh.userData.collider) this.interactionColliders.remove(animalMesh.userData.collider);
        animalMesh.traverse(disposeObject);
        this.animals.remove(animalMesh);
    }
    
    respawnTreeIfNeeded() {
        if (this.trees.children.length < this.initialTreeCount && this.createTreeAtRandomLocation()) {
            console.log("Árvore renasceu com sucesso!");
        }
    }
    respawnStoneIfNeeded() {
        if (this.stones.children.length < this.initialStoneCount && this.createStoneAtRandomLocation()) {
    ...
    respawnAnimalIfNeeded() {
        if (this.animals.children.length < this.initialAnimalCount) {
            ...
            if(y > WATER_LEVEL) {
               if (this.createAnimal(new THREE.Vector3(x, y + 0.4, z))) console.log("Animal renasceu com sucesso!");
            }
        }
    }
}