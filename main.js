...
export async function initializeGame(user) {
    ...
    loadingManager.onProgress = (url, itemsLoaded, itemsTotal) => {
        ...
        loadingProgressText.textContent = `${Math.round(progress * 100)}%`;
    };

    loadingManager.onLoad = () => {
        console.log("Todos os arquivos foram carregados!");
        console.log("Pré-compilando shaders...");
    ...
    if (user.uid !== 'offline-player') { 
        ...
        world = new World(scene, Math.random(), directionalLight, loadingManager); 
    ...
    world.loadAnimalModel(() => { 
        ...
        for(let i = 0; i < world.initialAnimalCount; i++) { 
            ...
            if(aY > WATER_LEVEL) { 
                ...
                if (newAnimal) animalsInstances.push(newAnimal); 
            } 
        } 
    });
...
function animate() {
    ...
    if (world && world.waterMesh) world.waterMesh.material.uniforms[ 'time' ].value += deltaTime;

    if (!isCraftingModalOpen && !isInteractionModalOpen && !isCampfireModalOpen && !isShelterModalOpen) {
        physics.update(camera, deltaTime); 
        animalsInstances.forEach(animal => animal.update(deltaTime)); 
    }
