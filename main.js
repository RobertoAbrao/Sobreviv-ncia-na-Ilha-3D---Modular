...
// ... (variáveis globais e craftableItems sem alteração) ...
...
// ... (resto da lógica do main.js sem alteração até o animate)
...
function animate() {
    ...
    if (!isCraftingModalOpen && !isInteractionModalOpen && !isCampfireModalOpen && !isShelterModalOpen) {
        ...
        // MODIFICADO: Chamada de update do animal simplificada
        animalsInstances.forEach(animal => animal.update(deltaTime, world)); 
        ...
        // MODIFICADO: Intersecta apenas com os colisores simples e invisíveis
        const objectsToIntersect = world.interactionColliders.children;
        const intersects = raycaster.intersectObjects(objectsToIntersect); // Não precisa ser recursivo

        if (intersects.length > 0 && intersects[0].distance < interactionHandler.maxInteractionDistance) {
            
            // MODIFICADO: Pega o objeto pai a partir do 'userData' do colisor
            ...
            if (highlightedObject !== interactableObject) {
                ...
                // A lógica de escala do highlight pode precisar de ajustes
                if (interactableObject.parent === world.trees) {
                    highlightMesh.scale.set(1.5, 4, 1.5);
                    highlightMesh.position.y = interactableObject.position.y + 2;
